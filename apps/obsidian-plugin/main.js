...
`;
  ...
  async readMemoryProposalFile(file) {
    ...
    if (proposalType && !normalizedProposalType.includes("memory-proposal")) {
      return null;
    }
    const created = this.firstString(data, ["created"]);
      ...
      for (const proposal of proposals) {
        ...
        if (proposal.approvalStatus === "pending") {
          ...
          apply.addEventListener("click", () => {
            new import_obsidian.Notice("Approved writeback is applied by Runtime through obs_wiki.apply_approved_writeback. The plugin does not write protected memory directly.");
          });
        }
      }
    }
