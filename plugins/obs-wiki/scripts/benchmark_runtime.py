...
from obs_wiki_shared import context_loader, context_pack, knowledge_lint, obsidian_runtime
from obs_wiki_shared.context_pack import build_context_pack
from obs_wiki_shared.knowledge_lint import build_lint_report
...
def create_fixture_vault(note_count: int, root: Path | None = None) -> Path:
    vault_path = Path(tempfile.mkdtemp(prefix="obs-wiki-benchmark-", dir=str(root) if root else None))
    created = datetime.now().isoformat(timespec="seconds")

    write_note(
        vault_path,
        "00_system/system.md",
        frontmatter("System", "system") + "# System\n\nobs-wiki benchmark fixture.\n",
    )
    write_note(
        vault_path,
        "00_system/index.md",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark obs-wiki context pack and lint runtime paths.")
