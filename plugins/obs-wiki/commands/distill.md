---
description: Distill the current work back into the active obs-wiki knowledge vault through session and log updates.
---

# obs-wiki Distill

Close the loop after meaningful work by creating a session note skeleton and updating the active obs-wiki knowledge vault deliberately.
...
```bash
python3 ~/.codex/plugins/obs-wiki/scripts/render_session_skeleton.py --apply --json
```
...
```bash
python3 ~/.codex/plugins/obs-wiki/scripts/apply_distill_updates.py "02_timeline/sessions/<session>.md" \
