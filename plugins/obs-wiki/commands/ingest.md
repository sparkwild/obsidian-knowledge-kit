---
description: Ingest a local file, directory, or URL into the active obs-wiki knowledge vault.
---

# obs-wiki Ingest

Bring new material into the active Obsidian vault through the obs-wiki raw-to-knowledge workflow.
...
```bash
python3 ~/.codex/plugins/obs-wiki/scripts/prepare_ingest_source.py "<source>" --json
```

If you are ready to write the source register and log entry:

```bash
python3 ~/.codex/plugins/obs-wiki/scripts/prepare_ingest_source.py "<source>" --apply --json
```

## Workflow

Follow the bundled `obs-wiki-ingest` skill.
...
```bash
python3 ~/.codex/plugins/obs-wiki/scripts/reconcile_source_register.py "<register-path>" --json
```

   - If you are ready to update the register state in place:

```bash
python3 ~/.codex/plugins/obs-wiki/scripts/reconcile_source_register.py "<register-path>" --apply --json
```
