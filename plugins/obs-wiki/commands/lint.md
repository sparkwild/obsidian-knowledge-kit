---
description: Run the obs-wiki knowledge lint workflow against the active Obsidian vault.
---
...
```bash
python3 ~/.codex/plugins/obs-wiki/scripts/lint_knowledge_vault.py --json
```

If you want a durable lint report note inside the vault:

```bash
python3 ~/.codex/plugins/obs-wiki/scripts/lint_knowledge_vault.py --apply --json
```
