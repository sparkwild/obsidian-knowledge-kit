...
```bash
python3 ~/.codex/plugins/vaultwright/scripts/check_codex_plugin.py --require-installed --json
```

3. Check required official skills:

```bash
python3 ~/.codex/plugins/vaultwright/skills/vaultwright-init/scripts/check_kepano_skills.py obsidian-cli obsidian-markdown
```

4. Check the current vault environment:

```bash
python3 ~/.codex/plugins/vaultwright/skills/vaultwright-init/scripts/check_obsidian_env.py --task init --json
```

5. Check the global AGENTS knowledge hint:

```bash
python3 ~/.codex/plugins/vaultwright/scripts/install_global_knowledge_hint.py --json
```
