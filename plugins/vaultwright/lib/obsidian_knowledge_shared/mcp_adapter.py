...
SERVER_NAME = "vaultwright-mcp-adapter"
...
def safe_note_path(raw_path: str) -> str:
    if raw_path.startswith("vaultwright://note/"):
        raw_path = raw_path.removeprefix("vaultwright://note/")
...
class VaultwrightMcpAdapter:
    ...
    def read_resource(self, uri: str) -> dict:
        vault_path = self.resolve_vault_path()
        resource_map = {
            "vaultwright://system": "00_system/system.md",
            "vaultwright://active-context": "01_ai_core/active_context.md",
            "vaultwright://index": "00_system/index.md",
        }
        if uri == "vaultwright://context-pack/latest":
            note_path = latest_note_in(vault_path, "01_ai_core/context_packs", name_prefix="context_")
        elif uri == "vaultwright://lint/latest":
            note_path = latest_note_in(vault_path, "00_system/reports", name_prefix="knowledge_lint_")
        elif uri.startswith("vaultwright://note/"):
...
TOOLS = [
    {
        "name": "vaultwright_status",
        "title": "Vaultwright Status",
        "description": "Report active vault, core note availability, and latest Vaultwright context/lint artifacts. Read-only.",
        "inputSchema": {"type": "object", "properties": {}, "additionalProperties": False},
        "annotations": {"readOnlyHint": True, "destructiveHint": False},
    },
    {
        "name": "vaultwright_query",
        "title": "Vaultwright Query",
    ...
    {
        "name": "vaultwright_lint",
        "title": "Vaultwright Lint",
    ...
    {
        "name": "vaultwright_read_note",
        "title": "Vaultwright Read Note",
    ...
    {
        "name": "vaultwright_review_queue",
        "title": "Vaultwright Review Queue",
...
RESOURCES = [
    {
        "uri": "vaultwright://system",
        "name": "system",
        "title": "Vaultwright System",
        "description": "The active vault system note.",
        "mimeType": TEXT_MARKDOWN,
    },
    {
        "uri": "vaultwright://active-context",
        "name": "active-context",
    ...
    {
        "uri": "vaultwright://context-pack/latest",
...
RESOURCE_TEMPLATES = [
    {
        "uriTemplate": "vaultwright://note/{path}",
        "name": "vault-note",
        "title": "Vault Note",
...
def prompt_messages(name: str, arguments: dict[str, Any]) -> dict:
    if name == "Vaultwright Query":
        query = str(arguments.get("query") or "").strip()
        if not query:
            raise McpError(-32602, "`query` is required.")
        text = (
            "Use the Vaultwright MCP adapter in read-only mode. "
            "Call `vaultwright_query` first, read only the highest-value candidate notes, "
            "and cite Obsidian note paths or block references when possible.\n\n"
            f"Question: {query}"
        )
        return {"description": "Vaultwright query workflow", "messages": [{"role": "user", "content": {"type": "text", "text": text}}]}
    if name == "Vaultwright Lint":
        text = (
            "Use the Vaultwright MCP adapter in read-only mode. "
            "Call `vaultwright_lint`, inspect errors/warnings, and recommend next actions without modifying the vault."
        )
        return {"description": "Vaultwright lint workflow", "messages": [{"role": "user", "content": {"type": "text", "text": text}}]}
    raise McpError(-32602, f"Unknown prompt: {name}")


class StdioMcpServer:
    def __init__(self, adapter: VaultwrightMcpAdapter) -> None:
    ...
    def dispatch(self, method: str, params: dict) -> dict:
        if method == "initialize":
            return {
                ...
                "serverInfo": {"name": SERVER_NAME, "title": "Vaultwright MCP Adapter", "version": SERVER_VERSION},
                "instructions": "Vaultwright MCP Adapter is read-only by default and delegates to Vaultwright shared runtime.",
            }
