...
obs-wiki 是一个面向 Obsidian vault 的 Codex 原生知识管家。它用于初始化知识库、导入外部资料、构建聚焦上下文包、优化已有知识结构，并让知识库保持可追溯、可审计、可维护。

对于 active vault 内的高频读写，obs-wiki 现在优先走共享的 Obsidian CLI runtime，只有在目标 vault 不是当前活动仓库，或者 CLI 明显做不到时才退回直接文件 I/O。

项目说明：

- 面向用户的产品名是 `obs-wiki`
- 内部包名、插件名和安装路径现在也统一使用 `obs-wiki`
- 当前仓库是 Codex 本地插件包与 runtime，不是 Obsidian community plugin
- active Obsidian vault 是唯一知识载体，不在 vault 外另建 raw/wiki 系统

## AI 对话优先 MCP，任务执行优先 CLI

obs-wiki 将 AI 对话入口和本地确定性执行层分开：
...
MCP 是 obs-wiki 面向 AI 对话工具的优先入口，用于暴露结构化的 tools、resources 和 prompts。MCP adapter 应该保持薄层设计：调用 obs-wiki CLI、scripts 或 shared runtime，不绕过 runtime，也不复制一套独立知识处理逻辑。

CLI 仍是确定性执行层，继续负责 active vault 检测、context pack、lint、ingest、distill、dashboard、report 等稳定任务。短期不把 MCP 作为核心依赖；它是面向 AI 客户端的可选增强层。当前 MVP 是只读 stdio MCP adapter，并委托 obs-wiki shared runtime 执行。
...
- `obs-wiki-init`：初始化 codex-native 知识库骨架。
- `obs-wiki-ingest`：将外部资料导入 `03_raw/`，并沉淀为稳定知识。
- `obs-wiki-query`：在回答知识问题前生成 Obsidian-native context pack。
- `obs-wiki-lint`：检查来源、断链、陈旧知识和日志追溯，不默认自动修复。
- `obs-wiki-refine`：优化知识结构、链接边界和状态信息。
- `plugins/obs-wiki`：符合 Codex 本地插件标准的自包含插件包。
- `lib/obs_wiki_shared`：共享的 preflight、官方 skill 检查与基础渲染逻辑。
...
运行一轮不会触碰真实 vault 的合成 runtime benchmark：
...
- 已提供 home-local 安装路径：`~/.codex/plugins/obs-wiki`
- 已提供 home-local marketplace：`~/.agents/plugins/marketplace.json`

## 项目文档

- [English README](./README.md)
- [Codex 任务方案](./docs/obs-wiki_Codex_Task_Brief.md)
- [Phase 2 MCP/CLI 性能方案](./docs/obs-wiki_Phase2_MCP_CLI_Performance_Plan.md)
- [MCP Adapter 设计](./docs/obs-wiki_MCP_Adapter_Design.md)
- [Runtime 性能评估](./docs/obs-wiki_Runtime_Performance_Evaluation.md)
- [Phase 2 执行清单](./docs/obs-wiki_Phase2_Execution_Checklist.md)
