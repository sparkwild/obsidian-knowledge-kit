# Vaultwright Runtime Performance Evaluation
...
Vaultwright should first stabilize Python plus `uv`, benchmark the current query/lint paths, and add incremental indexing only where measurements prove it is needed. MCP adapter language choice can be independent from the core runtime.
