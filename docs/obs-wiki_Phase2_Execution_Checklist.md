# obs-wiki Phase 2 Execution Checklist
...
Advance obs-wiki toward MCP-first AI conversation and CLI-first deterministic execution without implementing a complete MCP server or replacing the Python runtime.

## Checklist

- [x] Confirm the current branch and git status before editing.
- [x] Confirm `docs/obs-wiki_Phase2_MCP_CLI_Performance_Plan.md` exists.
- [x] Confirm current package paths use `plugins/obs-wiki`, `skills/obs-wiki-*`, and `~/.codex/plugins/obs-wiki`.
...
- MCP adapter work must call obs-wiki CLI/scripts or shared runtime instead of copying knowledge processing logic.
