# obs-wiki 下一阶段升级方案：MCP-first for AI Conversation, CLI-first for Execution

版本：2026-05-10
适用仓库：`sparkwild/obs-wiki`
建议执行分支：当前 `main` 已合并 `feat/obs-wiki-core-workflows`，后续可基于 `main` 新建下一阶段分支。
执行者：本地 Codex App
...
仓库已经从旧的 Obsidian knowledge prototype 收敛为 `obs-wiki`。当前产品展示名、插件名、安装路径和 skill 前缀都应统一为 `obs-wiki` / `obs-wiki`。

当前 `main` 已包含上一阶段核心成果：

- obs-wiki 品牌文案。
- `obs-wiki-query` skill。
- `obs-wiki-lint` skill。
- `query.md` / `lint.md` commands。
- `scripts/build_context_pack.py`。
- `scripts/lint_knowledge_vault.py`。
- `lib/obs_wiki_shared/context_pack.py`。
- `lib/obs_wiki_shared/knowledge_lint.py`。
- `lib/obs_wiki_shared/obsidian_runtime.py`。
- Obsidian Bases dashboard 渲染脚本。
- evidence-ready source register / claim-count / block-level evidence 相关增强。

因此下一阶段不是重做 Phase 1，而是在当前 `main` 的 `obs-wiki` 路径基础上继续推进。
...
obs-wiki MCP Adapter
        ↓
obs-wiki Query / Context / Lint / Review Tools
        ↓
obs-wiki CLI Runtime + Local Scripts
        ↓
...
MCP 是 AI 对话工具访问 obs-wiki 的统一协议层，负责让模型在对话中发现、选择和调用知识库资源与工具。

MCP 负责：

- 向 AI 客户端暴露 obs-wiki 的高层语义工具。
- 暴露只读 Resources，如 active context、index、latest lint report、context packs、review queues。
- 暴露 Prompts，如 obs-wiki Query / Lint / Ingest / Distill。
- 把用户的自然语言问题转换为可控的 obs-wiki 查询动作。
- 默认只读，写操作必须清晰标记并经过用户确认。

MCP 不负责：

- 直接绕过 obs-wiki runtime 任意读写文件。
- 直接替代 Obsidian vault。
- 建立 vault 外部的第二知识库。
- 让模型无确认地执行破坏性操作。
...
1. 让 obs-wiki 在 AI 对话工具中成为可发现、可调用、可解释的知识库入口。
...
6. 形成“高层 MCP tool -> obs-wiki runtime -> Obsidian CLI / vault”的稳定调用链。
...
- MCP adapter 只暴露高层 obs-wiki 语义工具，不暴露任意文件系统命令。
- 默认只读。
- 写入工具必须以 `apply`、`write`、`distill`、`ingest` 等语义明确命名。
- 破坏性操作不进入 MVP。
- MCP 工具返回结构化结果，必须包含 evidence/source/path/status 等可审计字段。
- MCP 不直接决定稳定知识写回；写回仍走 obs-wiki CLI/runtime 并产生 log/session/report。
...
- `obs-wiki://longterm-context`
- `obs-wiki://index`
- `obs-wiki://log/recent`
- `obs-wiki://context-pack/latest`
- `obs-wiki://lint/latest`
- `obs-wiki://dashboard/knowledge-inbox`
- `obs-wiki://dashboard/unverified-claims`
- `obs-wiki://dashboard/stale-concepts`
- `obs-wiki://note/{path}`
- `obs-wiki://search/{query}`
...
- MCP adapter 可以作为薄层，调用现有 obs-wiki scripts/runtime。
- 不要求重写 Python。
...
- 会混淆 MCP adapter 与 obs-wiki runtime。
- 对性能瓶颈没有充分证据。
...
目标：让 `main`、feature branch 和 `obs-wiki` 命名状态明确。

任务：

- 确认当前工作分支没有和 `main` 分叉。
- 如果出现分叉，先报告状态和建议，不强制 merge/rebase。
- 修正 `plugin.json` 中 homepage / repository / websiteURL / privacyPolicyURL / termsOfServiceURL 指向新仓库 `sparkwild/obs-wiki`。
- README 中确认产品名、插件名、安装路径都统一为 obs-wiki / `obs-wiki`。
- `check_codex_plugin.py` 继续检查 `plugins/obs-wiki`、`skills/obs-wiki-*` 和 `~/.codex/plugins/obs-wiki`。
...
- 明确 MCP 只暴露高层 obs-wiki tools/resources/prompts。
- 明确 read-only default 和 write confirmation。
- 明确不创建外部 raw/wiki 系统。

### PR C：MCP Adapter MVP

目标：让 AI 对话工具能通过 MCP 查询 obs-wiki。该 PR 需要单独确认后再执行，不能由文档补充任务直接实现。
...
- `obs_wiki_status`
- `obs_wiki_query`
- `obs_wiki_search_notes`
- `obs_wiki_read_note`
- `obs_wiki_list_review_queue`
- `obs_wiki_lint_preview`
...
- `obs_wiki_query` 能返回 context pack summary。
- `obs_wiki_read_note` 不能读取 vault 外路径。
- 默认不会写入 vault。
...
5. CLI-first for execution：本地确定性任务继续通过 obs-wiki scripts/runtime 和 Obsidian CLI 执行。
...
- obs-wiki 品牌和仓库 URL 在 README / plugin manifest / docs 中一致。
- MCP/CLI 分层架构文档完成。
...
- 当前仓库分支：`sparkwild/obs-wiki`, `main`
- 当前关键文件：
  - `README.md`
  - `plugins/obs-wiki/.codex-plugin/plugin.json`
  - `skills/obs-wiki-query/SKILL.md`
  - `skills/obs-wiki-lint/SKILL.md`
  - `lib/obs_wiki_shared/context_pack.py`
  - `lib/obs_wiki_shared/knowledge_lint.py`
  - `lib/obs_wiki_shared/obsidian_runtime.py`
- MCP 官方文档：
