...
请先完整阅读本文档，然后在当前仓库中执行“Phase 1：obs-wiki 品牌与边界升级”和“Phase 2：Query / Context Pack 工作流”。
...
- 将产品展示名、内部包名、插件名、skill 名和安装路径统一收敛到 `obs-wiki` / `obs-wiki`。
...
**obs-wiki — A Codex-native steward for Obsidian vaults.**

### 1.4 中文定位

**obs-wiki：面向 Obsidian vault 的 Codex 原生知识管家。**
...
- `plugins/obs-wiki/`：repo-local Codex plugin package。
- `plugins/obs-wiki/.codex-plugin/plugin.json`：Codex 插件 manifest。
- `plugins/obs-wiki/commands/`：Codex 命令入口。
- `lib/obs_wiki_shared/`：共享 runtime。
...
## Phase 1：obs-wiki 品牌与 Obsidian-native 边界升级

### 目标

把产品展示名、内部包名、插件名和安装路径统一升级为 obs-wiki / obs-wiki。强化全仓库约束：不要脱离 Obsidian vault，不要另起外部 raw/wiki 系统。
...
- `plugins/obs-wiki/agents/openai.yaml`
- `scripts/install_global_knowledge_hint.py`
- `lib/obs_wiki_shared/render_core_notes.py`
- `skills/obs-wiki-init/SKILL.md`
- `skills/obs-wiki-ingest/SKILL.md`
- `skills/obs-wiki-refine/SKILL.md`
- `plugins/obs-wiki/commands/setup.md`
- `plugins/obs-wiki/commands/start.md`
- `plugins/obs-wiki/commands/init.md`
- `plugins/obs-wiki/commands/ingest.md`
- `plugins/obs-wiki/commands/refine.md`
- `plugins/obs-wiki/commands/distill.md`
- `plugins/obs-wiki/commands/doctor.md`

### 产品文案要求

- 展示名使用 `obs-wiki`。
- 插件、skill、安装路径和文档统一使用 `obs-wiki`。
- README 要解释：obs-wiki 是 Codex-native steward for Obsidian vaults。
...
- `interface.displayName` 使用 `obs-wiki`。
...
- 插件内部 `name` 使用 `obs-wiki`，并与安装路径和 marketplace 统一。
...
- README 中主品牌为 obs-wiki。
- Codex plugin UI 展示名为 obs-wiki。
...
- `plugins/obs-wiki/commands/query.md`
...
- `obs-wiki-query` skill 存在并被插件检查脚本识别。
...
- `docs/obs-wiki_Phase2_MCP_CLI_Performance_Plan.md`
- `docs/obs-wiki_MCP_Adapter_Design.md`
- `docs/obs-wiki_Runtime_Performance_Evaluation.md`
- `docs/obs-wiki_Phase2_Execution_Checklist.md`
...
- `plugins/obs-wiki/commands/lint.md`
...
- `obs-wiki-lint` skill 存在并被插件检查脚本识别。
- lint 默认只读。
...
- README 英文和中文都能表达 obs-wiki 定位。
- plugin display name 为 obs-wiki。
...
请在当前仓库内实现 obs-wiki 第一批演进：

1. 将用户展示层品牌、内部包名、插件 package 名和安装路径统一为 `obs-wiki` / `obs-wiki`。
2. 强化所有相关文档、global hint、system note 模板中的 Obsidian-native 边界：active vault 是知识载体，不要另建外部 raw/wiki 系统。
3. 新增 `obs-wiki-query` skill、`query.md` command 和 context pack 生成能力。
