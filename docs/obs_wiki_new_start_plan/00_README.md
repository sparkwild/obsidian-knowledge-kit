...
执行者：本地 Codex App
目标：将 obs-wiki 作为一个全新的 Obsidian 原生插件产品重新设计与实现。
...
> obs-wiki 是一个 Obsidian 原生插件，让 Agent 使用 Obsidian vault 作为外置长期记忆库和知识库。Agent 通过 MCP / runtime 调用记忆；Obsidian 插件负责审计、复核、权限、资料分析入口和可视化监督。
...
优先实现 Obsidian 原生插件骨架与 Agent-first 记忆工作流，而不是继续优化旧的 Codex 插件体验。
