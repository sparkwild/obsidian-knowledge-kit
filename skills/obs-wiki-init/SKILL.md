---
name: obs-wiki-init
description: Initialize an Obsidian knowledge vault into the codex-native obs-wiki knowledge system. Use when a vault has not been set up yet, when core system notes are missing, or when the user wants to bootstrap a new Obsidian knowledge base. Before doing anything, verify Obsidian is running, the obsidian CLI works, and the official kepano/obsidian-skills dependencies exist. The active Obsidian vault is the only knowledge carrier; do not create an external raw/wiki directory system.
---
